    ...
    <fieldset class="wps_wpr_each_section">
        <div class="wps_wpr_refrral_code_copy">
            ...
            <button class="wps_wpr_btn_copy wps_tooltip" data-clipboard-target="#wps_wpr_copy" aria-label="copied">
                <span
                    class="wps_tooltiptext"><?php esc_html_e( 'Copy', 'points-and-rewards-for-woocommerce' ); ?></span>
                <img src="<?php echo esc_url( WPS_RWPR_DIR_URL . 'public/images/copy.png' ); ?>"
...
<?php
	...
	public function wps_wpr_update_points_details( $user_id, $type, $points, $data ) {
		...
		$today_date = date_i18n( 'Y-m-d h:i:s a' );
	...
	public function wps_wpr_woocommerce_order_status_changed( $order_id, $old_status, $new_status ) {
		...
		if ( in_array( $old_status, $mwb_wpr_array, true ) && ( 'cancelled' === $new_status || 'refunded' === $new_status ) ) {
			...
			if ( ! isset( $pre_wps_check ) || 'done' != $pre_wps_check ) {
				...
				if ( array_key_exists( 'pro_conversion_points', $wps_points_log ) ) {
    
					$today_date = date_i18n( 'Y-m-d h:i:s a' );
				
					foreach ($wps_points_log['pro_conversion_points'] as $key => $value) {
						
						if ( ! isset( $pre_wps_check ) || 'done' != $pre_wps_check ) {
						
							if ( $value['refered_order_id'] == $order_id ) {
				
								$value_to_withdraw = $value['pro_conversion_points'];
				
								$wps_total_points_par = get_user_meta( $user_id, 'wps_wpr_points', true );
				
								$wps_points_newly_updated = (int) ( $wps_total_points_par - $value_to_withdraw );
				
								$wps_refer_withdraw_points = get_user_meta( $user_id, 'points_details', true );
				
								if ( isset( $wps_refer_withdraw_points['deduction_currency_spent'] ) && ! empty( $wps_refer_withdraw_points['deduction_currency_spent'] ) ) {
									
									$wps_par_withdraw_purchase = array();
								
									$wps_par_withdraw_purchase = array(
								
										'deduction_currency_spent' =>  $value_to_withdraw,
								
										'date' => $today_date,
								
									);
								
									$wps_refer_withdraw_points['deduction_currency_spent'][] = $wps_par_withdraw_purchase;
							
								} else {
				
									if ( ! is_array( $wps_refer_withdraw_points ) ) {
										$wps_refer_withdraw_points = array();
									}
								...
								update_user_meta( $user_id, 'wps_wpr_points', $wps_points_newly_updated );
								update_user_meta( $user_id, 'points_details', $wps_refer_withdraw_points );
								break;
							}
...
<div class="wps_wpr_apply_custom_points">
    ...
    <button class="button wps_cart_points_apply" name="wps_cart_points_apply" id="wps_cart_points_apply"
        ...
        data-point="<?php echo esc_html( $get_points ); ?>" data-id="<?php echo esc_html( $user_id ); ?>"
...
<div class="wps_wpr_apply_custom_points">
    ...
    <div class="wps_wpr_avada_wrap checkout_coupon">
        <h2 class="promo-code-heading fusion-alignleft">
            <?php esc_html_e( 'Have A Points?', 'points-and-rewards-for-woocommerce' ); ?></h2>
    ...
    <div class="custom_point_checkout woocommerce-info wps_wpr_checkout_points_class">
        ...
        <p><?php esc_html_e( 'You require :', 'points-and-rewards-for-woocommerce' ); ?>
            ...
            <?php esc_html_e( 'more points to get redeem', 'points-and-rewards-for-woocommerce' ); ?></p>
    </div>
    <?php
				}
			}
    ...
    <p class="wps_wpr_heading">
        <?php echo esc_html__( 'Convert Points to Currency  Wallet Conversion', 'ultimate-woocommerce-points-and-rewards' ); ?>
    </p>
    <fieldset class="wps_wpr_each_section">
        <p>
        ...
        <form id="points_wallet" enctype="multipart/form-data" action="" method="post">
            ...
            <input type="number" class="woocommerce-Input woocommerce-Input--number input-number"
                name="wps_custom_number" min="1" id="wps_custom_wallet_point_num" style="width: 160px;">

            <input type="button" name="wps_wpr_custom_wallet" id="wps_wpr_custom_wallet"
                class="wps_wpr_custom_wallet button"
                value="<?php esc_html_e( 'Redeem to Wallet', 'ultimate-woocommerce-points-and-rewards' ); ?>"
